...
/**
 * @brief Prints the printf style message to the stderr and exit the program with error.
 */
...
#define ASSERT_EQ(v, expected, fmt, ...) ASSERT((v) == (expected), "%lld != %lld", (long long)(v), (long long)(expected))
//#define ASSERT_NE(v, expected, fmt, ...) ASSERT((v) != (expected))
 ...
 * @details Assume buf is zero terminated. Buf must be at least one byte in size.
 */
 ...
 * @param value - pointer to last value seen in enum. If this item do not contain explicit value then
 *          the *value +1 is used. Anyhow *value is set to the current used value.
 * @return P4_Error 
 * @attention values above params->params->max_enum_value are ignored.
 */
P4_Error
EmitEnumItem(P4_Node *root, FILE *file, int *value)
{
 ...
 */
P4_Error
EmitEnumList(char *identifier, P4_Node *root, FILE *file, ErrStr *errstr)
{
 ...
 */
P4_Error
ProcessEnumType(P4_Node *root, FILE *file, ErrStr *errstr)
{
 ...
 */
P4_Error
ProcessEnumDef(P4_Node *root, FILE *file, ErrStr *errstr)
{
    ...
    node = node->next;
    if (strcmp(node->rule_name, "SEMI")) {
        //Panic("Enum def without ';' pos[%d:%d]", root->slice.start.lineno, root->slice.start.offset);
 ...
 */
P4_Error
ProcessRootEnums(P4_Node *root, FILE *file, ErrStr *errstr)
{
    // enum_value = (identifier EQ integer) / identifier;
 ...
 * @details This is a callback function to be used by P4_InspectSourceAst() (see main).
 */
